<x-app-layout>
    <x-slot name="header">
        <h1 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Slots disponibles para :date', ['date' => $date->isoFormat('dddd, D [de] MMMM [de] YYYY')]) }}
        </h1>
    </x-slot>
                    ...
                    <div class="mb-6 bg-white dark:bg-gray-800">
                        @if(auth()->user()->hasCredits())
                            <form action="{{ route('slots.book', ['business' => $business, 'slot' => $slot]) }}" method="POST">
                                ...
                                <button
                                    type="submit"
                                    class="bg-green-600 dark:bg-green-400 text-white dark:text-gray-900 font-semibold py-2 px-4 rounded hover:bg-green-500 dark:hover:bg-green-300"
                                >
                                    Reservar
                                </button>
                            </form>
